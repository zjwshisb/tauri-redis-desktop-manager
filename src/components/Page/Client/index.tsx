...
import { Button, Col, Input, Modal, Row, Space, Spin, Table, Tooltip, message } from 'antd'
...
function clientColumn (title: string, tooltip: string): ColumnType<Client> {
  return {
    ...
    title: <Tooltip title={tooltip}>
            {title}<QuestionCircleOutlined className='text-sm ml-2'/>
        </Tooltip>
  }
}
...
const Index: React.FC<{
  ...
  const { data: clientStr, loading, fetch } = useRequest<string>('client/list', connection.id)
  ...
  const handleKill = React.useCallback((id: string) => {
    Modal.confirm({
      title: 'notice',
  ...
  return <Spin spinning={loading}>
        ...
        <Row>
            <Col span={24}>
                ...
                bordered
                dataSource={clients} columns={[
                  clientColumn('id', 'a unique 64-bit client ID'),
                  clientColumn('addr', 'address/port of the client'),
                  clientColumn('laddr', 'address/port of local address client connected to (bind address)'),
                  ...
                  clientColumn('ssub', 'number of shard channel subscriptions. Added in Redis 7.0.3'),
                  ...
                  clientColumn('qbuf-free', 'free space of the query buffer (0 means the buffer is full)'),
                  clientColumn('argv-mem', 'argv-mem'),
                  clientColumn('multi-mem', 'memory is used up by buffered multi commands. Added in Redis 7.0'),
                  clientColumn('obl', 'output buffer length'),
                  clientColumn('oll', 'output list length (replies are queued in this list when the buffer is full)'),
                  clientColumn('omem', 'output buffer memory usage'),
                  clientColumn('tot-mem', 'total memory consumed by this client in its various buffers'),
                  ...
                  clientColumn('resp', 'client RESP protocol version. Added in Redis 7.0'),
                  {
                    ...
                    render (_, record) {
                      return <Space>
                            ...
                            style={actionIconStyle} className='hover:cursor-pointer'/>
                        </Space>
                    }
                  }
                ]}>
